...
Outgoing.prototype.placeOrder = function (id, contract, order) {
  ...
  var version = (this._controller._serverVersion < C.MIN_SERVER_VER.NOT_HELD ? 27 : 45);
  ...
  if (this._controller._serverVersion >= C.MIN_SERVER_VER.ALGO_ID) {
    args.push(order.algoId);
  }
  ...
  args.push('');  // TODO send orderMiscOptions parameter

  if (this._controller._serverVersion >= C.MIN_SERVER_VER.ORDER_SOLICITED) {
    args.push(order.solicited);
  }
  
  if (this._controller._serverVersion >= C.MIN_SERVER_VER.RANDOMIZE_SIZE_AND_PRICE) {
    args.push(order.randomizeSize);
    args.push(order.randomizePrice);
  }

  if (this._controller._serverVersion >= C.MIN_SERVER_VER.PEGGED_TO_BENCHMARK) {
    args.push(0);

    args.push(order.adjustedOrderType)
    args.push(order.triggerPrice || Number.MAX_VALUE)
    args.push(order.lmtPriceOffset || Number.MAX_VALUE)
    args.push(order.adjustedStopPrice || Number.MAX_VALUE )
    args.push(order.adjustedStopLimitPrice || Number.MAX_VALUE)
    args.push(order.adjustedTrailingAmount || Number.MAX_VALUE)
    args.push(order.adjustableTrailingUnit)
  }

  if (this._controller._serverVersion >= C.MIN_SERVER_VER.EXT_OPERATOR) {
    args.push(order.extOperator);
  }

  if (this._controller._serverVersion >= C.MIN_SERVER_VER.SOFT_DOLLAR_TIER) {
    if (_.isPlainObject(contract.softDollarTier) && !_.isEmpty(contract.softDollarTier)) {
      args.push(contract.softDollarTier.name);
      args.push(contract.softDollarTier.value);
    } else {
      args.push(undefined);
      args.push(undefined);
    }
