...
Incoming.prototype._OPEN_ORDER = function () {
  ...
  order.totalQuantity = (this._controller._serverVersion >= C.MIN_SERVER_VER.FRACTIONAL_POSITIONS) ? this.dequeueFloat() : this.dequeueInt();
  ...
  if (this._controller._serverVersion >= C.MIN_SERVER_VER.MODELS_SUPPORT) {
  ...
  if (version >= 34) {
    ...
    order.randomizePrice = this.dequeue();
  }

  if (this._controller._serverVersion >= C.MIN_SERVER_VER.PEGGED_TO_BENCHMARK) {
    if (order.orderType == C.ORDER_TYPE.PEG_BENCH) {
      order.referenceContractId = this.dequeue();
    ...
    var nConditions = this.dequeue(); 
    if (nConditions > 0){
      // TODO
    }

    order.adjustedOrderType = this.dequeue();
    order.triggerPrice = this.dequeueFloat() || Number.MAX_VALUE;
    order.trailStopPrice = this.dequeueFloat() || Number.MAX_VALUE;
    order.lmtPriceOffset = this.dequeueFloat() || Number.MAX_VALUE;
    order.adjustedStopPrice = this.dequeueFloat() || Number.MAX_VALUE;
    order.adjustedStopLimitPrice = this.dequeueFloat() || Number.MAX_VALUE;
    order.adjustedTrailingAmount =  this.dequeueFloat() || Number.MAX_VALUE;
    order.adjustableTrailingUnit = this.dequeue();
  }

  if (this._controller._serverVersion >= C.MIN_SERVER_VER.SOFT_DOLLAR_TIER) {
    order.softDollarTier = {name: this.dequeue(), value: this.dequeue(), displayName: this.dequeue()};
  }
...
Incoming.prototype._ORDER_STATUS = function () {
  ...
  var filled = (this._controller._serverVersion >= C.MIN_SERVER_VER.FRACTIONAL_POSITIONS) ? this.dequeueFloat() : this.dequeueInt();
  var remaining = (this._controller._serverVersion >= C.MIN_SERVER_VER.FRACTIONAL_POSITIONS) ? this.dequeueFloat() : this.dequeueInt();
...
Incoming.prototype._PORTFOLIO_VALUE = function () {
  ...
  var position = (this._controller._serverVersion >= C.MIN_SERVER_VER.FRACTIONAL_POSITIONS) ? this.dequeueFloat() : this.dequeueInt();
...
Incoming.prototype._POSITION = function () {
  ...
  var pos = (this._controller._serverVersion >= C.MIN_SERVER_VER.FRACTIONAL_POSITIONS) ? this.dequeueFloat() : this.dequeueInt();
  var avgCost = 0;
  if (version >= 3) {
